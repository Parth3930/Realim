...
export function CharacterController({ onFollow, broadcast }: { onFollow?: (x: number, y: number) => void, broadcast?: (msg: any) => void }) {
    ...
    // Find my character
    const myCharId = Object.values(store.elements).find(el => el.type === 'character' && el.playerId === userId)?.id;
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            const platforms = allElements.filter(el => el.type !== 'character' && el.type !== 'path');
            ...
            for (const platform of platforms) {
                const baseW = platform.width || (platform.type === 'text' ? (platform.content.length * 10) : platform.type === 'sticky' ? 220 : 100);
                const scale = platform.scale || 1;
                const effectiveW = baseW * scale;

                const baseRotation = platform.rotation || (platform.type === 'sticky' ? 1 : 0);
                    ...
                    for (let i = 0; i < pathEl.points.length - 1; i++) {
                        ...
                        for (const fx of [feetLX, feetRX]) {
                            ...
                            if (fx >= minX - 10 && fx <= maxX + 10) {
                                ...
                                if (p.vy >= 0 && feetY >= targetY - 10 && feetY <= targetY + 20) {
            ...
            if (broadcast && (p.vx !== 0 || p.vy !== 0 || !p.isGrounded || Date.now() - p.lastSync > 100)) {
                broadcast({
                    type: 'UPDATE_ELEMENT',
    ...
    const K = ({ children }: { children: React.ReactNode }) => (
        <span className="w-6 h-6 flex items-center justify-center bg-white/10 rounded border border-white/10 text-xs font-mono">{children}</span>
    );
        ...
        <>
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2">
                <div className="relative group">
                    <div className={`absolute inset-0 bg-violet-600 blur-xl opacity-40 rounded-full transition-opacity group-hover:opacity-60 ${isPlaying ? 'bg-red-600' : ''}`} />
                    <Button
                        size="lg"
